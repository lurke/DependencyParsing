...
%    Our results have somewhat worse accuracy than that in the paper--we suspect a few reasons for this: for one, SVMs with polynomial kernel functions like those used in the paper weren't working at all for us, so we only use linear SVMs, which may have worse accuracy 
